---
title: "Bison results"
subtitle: "_152 bison mtDNA sequences sampled from the present to 55,182 years BP (602bp)._"
...
```{r rsetup, include=FALSE}
    ...
    knitr::opts_chunk$set(tidy=FALSE, cache=FALSE, cache.path=paste0(path,"cache/"),
                          dev="pdf", fig.path=paste0(path,"figures/"), 
                          message=FALSE, error=FALSE, warning=FALSE, echo=FALSE, results='hide')
    
    logpath   <- paste0(path,"output/")
    seeds     <- c(125, 126, 127)
...
```{r functions, include=FALSE}
  ...
  smin <- function(x, digits=2, ...) {
    if (length(x) == 0) {
        return ("-")
    } else {
        return (round(min(x, ...), digits))
    }
  }
...
```{r convergence.BSP20, cache=TRUE, fig.width=3.5, fig.height=2, fig.cap="test"}
 
    model.BSP20    <- "Bison.HKY+G+F.BSP20"
    ...
    mcmc.combined.BSP20 <- mcmc(as.matrix(mcmc.trace.BSP20, iters = TRUE, chains = TRUE), thin=thin(mcmc.trace.BSP20))
    ...
    varnames(popSizesGridded.BSP20) <- paste0("bPopSizeGridded_",varnames(popSizesGridded.BSP20))
     
    # Check convergence
    par(mar=c(3,5,1,0)+0.1, cex=0.6)
    mcmc.nonstat.BSP20     <- checkESS(mcmc.combined.BSP20,   cutoff=ESSlimit, ignored=c("CHAIN","ITER", varnames(mcmc.trace.BSP20)[27:nvar(mcmc.trace.BSP20)]), 
    ...
    writeLog(mcmc.combined.BSP20,   paste0(logpath, model.BSP20, ".combined.log"))
    writeLog(popSizesGridded.BSP20, paste0(logpath, model.BSP20, ".popSizesGridded.combined.log"))
...
```{r convergence.BESP20_1, cache=TRUE, fig.width=3.5, fig.height=2}
    ...
    model.BESP20_1    <- "Bison.HKY+G+F.BESP20_1"
    ...
    mcmc.combined.BESP20_1 <- mcmc(as.matrix(mcmc.trace.BESP20_1, iters = TRUE, chains = TRUE))  
    ...
    varnames(popSizesGridded.BESP20_1) <- paste0("bPopSizeGridded_",varnames(popSizesGridded.BESP20_1))

    sampIntensities.BESP20_1    <- getLogFileSubset(mcmc.combined.BESP20_1, "bSamplingIntensity$")
     
    # Check convergence
    par(mar=c(3,5,1,0)+0.1, cex=0.6)
    mcmc.nonstat.BESP20_1     <- checkESS(mcmc.combined.BESP20_1,   cutoff=ESSlimit, ignored=c("CHAIN","ITER",varnames(mcmc.trace.BESP20_1)[27:(nvar(mcmc.trace.BESP20_1)-2)]), 
                                           plot=TRUE, log='y', ylim=c(1,10000), title="General parameters", plot.grid=TRUE)
    ...
    writeLog(mcmc.combined.BESP20_1,   paste0(logpath, model.BESP20_1, ".combined.log"))
    writeLog(popSizesGridded.BESP20_1, paste0(logpath, model.BESP20_1, ".popSizesGridded.combined.log"))
...
```{r convergence.BESP20_12, cache=TRUE, fig.width=3.5, fig.height=2}
    ...
    model.BESP20_12    <- "Bison.HKY+G+F.BESP20_12"  
    ...
    mcmc.combined.BESP20_12 <- mcmc(as.matrix(mcmc.trace.BESP20_12, iters = TRUE, chains = TRUE))
    ...
    varnames(popSizesGridded.BESP20_12) <- paste0("bPopSizeGridded_",varnames(popSizesGridded.BESP20_12))
    ...
    # Check convergence
    par(mar=c(3,5,1,0)+0.1, cex=0.6)
    mcmc.nonstat.BESP20_12     <- checkESS(mcmc.combined.BESP20_12,   cutoff=ESSlimit, ignored=c("CHAIN","ITER", varnames(mcmc.trace.BESP20_12)[27:nvar(mcmc.trace.BESP20_12)]), 
                                 plot=TRUE, log='y', ylim=c(1,10000), title="General parameters", plot.grid=TRUE)
...
```{r results='asis'} 
    ...
    nonstat <- data.frame(row.names = c("PopSize", "Gridded PopSize", "Other parameters"), 
                          BSP20     = c(length(pop.nonstat.BSP20),     length(popgrid.nonstat.BSP20),     length(mcmc.nonstat.BSP20)),
                          BESP20_1  = c(length(pop.nonstat.BESP20_1),  length(popgrid.nonstat.BESP20_1),  length(mcmc.nonstat.BESP20_1)),
                          BESP20_12 = c(length(pop.nonstat.BESP20_12), length(popgrid.nonstat.BESP20_12), length(c(mcmc.nonstat.BESP20_12, samp.nonstat.BESP20_12))))
    ess     <- data.frame(row.names = c("PopSize", "Gridded PopSize", "Other parameters"), 
                          BSP20     = c(smin(pop.nonstat.BSP20),     smin(popgrid.nonstat.BSP20),     smin(mcmc.nonstat.BSP20)),
                          BESP20_1  = c(smin(pop.nonstat.BESP20_1),  smin(popgrid.nonstat.BESP20_1),  smin(mcmc.nonstat.BESP20_1)),
                          BESP20_12 = c(smin(pop.nonstat.BESP20_12), smin(popgrid.nonstat.BESP20_12), smin(c(mcmc.nonstat.BESP20_12, samp.nonstat.BESP20_12))))
    colnames(nonstat) <- colnames(ess) <- c("BSP 20", "BESP 20/1", "BESP 20/12")
    
    knitr::kable(nonstat, caption = "Number of nonstationary parameters for different models and subsets of parameters.")
    knitr::kable(ess,     caption = "Lowest ESS values for different models and subsets of parameters.")
```


For checking convergence and combining chains a burn-in of `r burnin*100`% was used, and chains are thinned by a factor of `r thinning`, effectively sampling from the MCMC chains every $`r thin(mcmc.trace.BESP20_12)`$ states and resulting in chains with `r niter(mcmc.combined.BESP20_12)` posterior samples.

For convergence only the general parameters and the gridded population size parameters are important. The logged population size and group size parameters are not always expected to mix well. 
...
```{r bison-results-BESP20_12, fig.width=10, fig.height=7, eval=TRUE, fig.cap="(A) Density of sequence sampling dates through time for the alignment of 152 bison mtDNA sequences that we used. Blue dots indicate stripcharts of individual samples for each sampling epoch. The height of the stripcharts are equal to the number of samples in each epoch. Small tick marks on the x-axis represent epoch times. Grey shading indicates cool periods in the Earth's climate (from the present: Younger Dryas, Marine Isotope Stages (MIS) 2, MIS 4). The two cross hatched areas delimit the time of the last glacial maximum (~26.5-19 ka BP) and approximate time of substantial human settlement of the Americas (~16-14 ka BP). (B) Median (solid line) and 95% highest posterior density (HPD) intervals (shaded areas) for the genetic diversity estimates (N_e*tau) through time of the 12-epoch BESP. (C) Median (solid line) and 95% HPD intervals (shaded areas) of the estimated sampling intensities (beta) for each sampling epoch of the 12-epoch BESP."} 
...
```{r bison-results-BSP20, fig.width=10, fig.height=7, eval=TRUE, fig.cap="(A) Density of sequence sampling dates through time for the alignment of 152 bison mtDNA sequences that we used. Blue dots indicate stripcharts of individual samples for each sampling epoch. The height of the stripcharts are equal to the number of samples in each epoch. Small tick marks on the x-axis represent epoch times. Grey shading indicates cool periods in the Earth's climate (from the present: Younger Dryas, Marine Isotope Stages (MIS) 2, MIS 4). The two cross hatched areas delimit the time of the last glacial maximum (~26.5-19 ka BP) and approximate time of substantial human settlement of the Americas (~16-14 ka BP). (B) Median (dotted line) and 95% highest posterior density (HPD) intervals (shaded areas) for the genetic diversity estimates (N_e*tau) through time of the BSP."} 
...
```{r bison-results-BESP20_12-MCCtree, fig.width=7.5, fig.height=9, fig.cap="MCC tree of the alignment of 152 bison mtDNA sequences estimated under the 12-epoch BESP. Grey shading indicates cool periods in the Earth's climate (from the present: Younger Dryas, Marine Isotope Stages (MIS) 2, MIS 4). The two cross hatched areas delimit the time of the last glacial maximum (~26.5-19 ka BP) and approximate time of substantial human settlement of the Americas (~16-14 ka BP)."} 
    ...
    model.BESP20_12    <- "Bison.HKY+G+F.BESP20_12"
    phylo <- read.nexus(paste0(logpath, model.BESP20_12, ".combined10_30000.MCC.tree"))
    tmrca <- max(getBranchingTimes(phylo))
...
```{r bison-results-BESP20_1-MCCtree, fig.width=7.5, fig.height=9, fig.cap="MCC tree of the alignment of 152 bison mtDNA sequences estimated under the single-epoch BESP. Grey shading indicates cool periods in the Earth's climate (from the present: Younger Dryas, Marine Isotope Stages (MIS) 2, MIS 4). The two cross hatched areas delimit the time of the last glacial maximum (~26.5-19 ka BP) and approximate time of substantial human settlement of the Americas (~16-14 ka BP)."} 

    model.BESP20_1    <- "Bison.HKY+G+F.BESP20_1"
    phylo <- read.nexus(paste0(logpath, model.BESP20_1, ".combined10_30000.MCC.tree"))
    tmrca <- max(getBranchingTimes(phylo))
...
```{r bison-results-BSP20-MCCtree, fig.width=7.5, fig.height=9, fig.cap="MCC tree of the alignment of 152 bison mtDNA sequences estimated under the BSP. Grey shading indicates cool periods in the Earth's climate (from the present: Younger Dryas, Marine Isotope Stages (MIS) 2, MIS 4). The two cross hatched areas delimit the time of the last glacial maximum (~26.5-19 ka BP) and approximate time of substantial human settlement of the Americas (~16-14 ka BP)."} 

    model.BSP20    <- "Bison.HKY+G+F.BSP20"
    phylo <- read.nexus(paste0(logpath, model.BSP20, ".combined10_30000.MCC.tree"))
    tmrca <- max(getBranchingTimes(phylo))
