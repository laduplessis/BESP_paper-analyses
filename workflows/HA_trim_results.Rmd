---
title: "H3N2 HA (trimmed) results"
subtitle: "_637 HA sequences sampled between 1993.06 and 2005.25 (1,698bp)._"
...
```{r convergence.BSP40, cache=TRUE, fig.width=3.5, fig.height=2}
    ...
    writeLog(mcmc.combined.BSP40,   paste0(logpath, model.BSP40, ".combined.log"))
    writeLog(popSizesGridded.BSP40, paste0(logpath, model.BSP40, ".popSizesGridded.combined.log"))
...
```{r convergence.BESP40_1, cache=TRUE, fig.width=3.5, fig.height=2}
    ...
    writeLog(mcmc.combined.BESP40_1,   paste0(logpath, model.BESP40_1, ".combined.log"))
    writeLog(popSizesGridded.BESP40_1, paste0(logpath, model.BESP40_1, ".popSizesGridded.combined.log"))
```
...
```{r convergence.BESP40_12, cache=TRUE, fig.width=3.5, fig.height=2}
    ...
    writeLog(mcmc.combined.BESP40_12,   paste0(logpath, model.BESP40_12, ".combined.log"))
    writeLog(popSizesGridded.BESP40_12, paste0(logpath, model.BESP40_12, ".popSizesGridded.combined.log"))
```
...
```{r results='asis'} 
    ...
    nonstat <- data.frame(row.names = c("PopSize", "Gridded PopSize", "Other parameters"), 
                          BSP40     = c(length(pop.nonstat.BSP40),     length(popgrid.nonstat.BSP40),     length(mcmc.nonstat.BSP40)),
                          BESP40_1  = c(length(pop.nonstat.BESP40_1),  length(popgrid.nonstat.BESP40_1),  length(mcmc.nonstat.BESP40_1)),
                          BESP40_12 = c(length(pop.nonstat.BESP40_12), length(popgrid.nonstat.BESP40_12), length(c(mcmc.nonstat.BESP40_12, samp.nonstat.BESP40_12))))
    ess     <- data.frame(row.names = c("PopSize", "Gridded PopSize", "Other parameters"), 
                          BSP40     = c(smin(pop.nonstat.BSP40),     smin(popgrid.nonstat.BSP40),     smin(mcmc.nonstat.BSP40)),
                          BESP40_1  = c(smin(pop.nonstat.BESP40_1),  smin(popgrid.nonstat.BESP40_1),  smin(mcmc.nonstat.BESP40_1)),
                          BESP40_12 = c(smin(pop.nonstat.BESP40_12), smin(popgrid.nonstat.BESP40_12), smin(c(mcmc.nonstat.BESP40_12, samp.nonstat.BESP40_12))))
    colnames(nonstat) <- colnames(ess) <- c("BSP 40", "BESP 40/1", "BESP 40/12")
...
For checking convergence and combining chains a burn-in of `r burnin*100`% was used, and chains are thinned by a factor of `r thinning`, effectively sampling from the MCMC chains every $`r thin(mcmc.trace.BESP40_12)`$ states and resulting in chains with `r niter(mcmc.combined.BESP40_12)` posterior samples.
...
HA <- read.csv("../data/H3N2/HA_trim.csv")
...
- BESP 40/12 $\beta$: `r round(apply(sampIntensityHPD.BESP40_12, 1, mean),2)` (mean of median and 95\% HPD)
- BESP 40/1 $\beta$: `r round(sampIntensityHPD.BESP40_1,2)` (median and 95\% HPD)
...
```{r H3N2_trim-results-BESP40_12-vs-BSP40, fig.width=10, fig.height=7, fig.cap="(A)  Density of sequence sampling dates through time for the alignment of 637 A/H3N2 HA sequences from NY state that we analysed. Blue dots indicate stripcharts of individual samples for each season. The stripchart heights give the number of samples in each season. Grey shading indicates the approximate period of influenza observation in New York state during each season (epidemiological week 40, to week 20 in the next year). Cross-hatched seasons are those where A/H3N2 was not the dominant influenza virus subtype. (B) Median (solid/dotted line) and 95% highest posterior density (HPD) intervals (shaded areas) for the genetic diversity estimates (N_e*tau) through time. The BESP estimate is shown in blue and the BSP estimate is in red. (C) Median (solid line/dotted line) and 95% HPD intervals (shaded areas) of the estimated sampling intensities (beta) for each sampling epoch. The 12-epoch BESP estimates are shown in blue and a single-epoch (density-defined) estimate is in yellow."} 
...
```{r H3N2_trim-results-BESP40_12, fig.width=10, fig.height=7, eval=FALSE} 
...
```{r H3N2_trim-results-BESP40_1, fig.width=10, fig.height=7, eval=FALSE} 
...
```{r H3N2_trim-results-BSP40, fig.width=10, fig.height=7, eval=FALSE} 
...
# Samples
    ...
    density_1mo$y <- (density_1mo$y*sampleMax*0.8) / max(density_1mo$y)
    polygon(density_1mo, col=mPal(oxCols$red2,0.25), border=NA)
...
```{r H3N2_trim-results-BESP40_1-MCCtree, fig.width=7.5, fig.height=9, fig.cap="MCC tree of the alignment of 637 A/H3N2 HA sequences estimated under the single-epoch BESP. Grey shading indicates the approximate period of influenza observation in New York state during each season (epidemiological week 40, to week 20 in the next year). Cross-hatched seasons are those where A/H3N2 was not the dominant influenza virus subtype.", eval=TRUE} 

    model.BESP40_1    <- "HA.HKY+G+F.BESP40_1"
    phylo <- read.nexus(paste0(logpath, model.BESP40_1, ".combined30_30000.MCC.tree"))
    tmrca <- max(getBranchingTimes(phylo))
    maxTime    <- 2005.25
...
```{r H3N2_trim-results-BSP40-MCCtree, fig.width=7.5, fig.height=9, fig.cap="MCC tree of the alignment of 637 A/H3N2 HA sequences estimated under the single-epoch BESP. Grey shading indicates the approximate period of influenza observation in New York state during each season (epidemiological week 40, to week 20 in the next year). Cross-hatched seasons are those where A/H3N2 was not the dominant influenza virus subtype.", eval=TRUE} 

    model.BSP40    <- "HA.HKY+G+F.BSP40"
    phylo <- read.nexus(paste0(logpath, model.BSP40, ".combined30_30000.MCC.tree"))
    tmrca <- max(getBranchingTimes(phylo))
    maxTime    <- 2005.25
